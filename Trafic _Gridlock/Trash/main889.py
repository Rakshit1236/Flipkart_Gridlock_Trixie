...
p("=" * 60)
p("MAIN889 - 2 Param Sets Ensemble (sweet + regularized)")
p("=" * 60)
...
# 2 param sets: sweet spot (main888) + moderate
param_sets = [
    {'name': 'sweet', 'lgb': {'lr':0.025,'nl':90,'md':9,'ra':0.8,'rl':1.5,'mcs':20}, 'xgb': {'lr':0.025,'md':8,'ra':1.0,'rl':3.0,'mcw':5}, 'cb': {'lr':0.04,'d':8,'l2':2.0}},
    {'name': 'mid', 'lgb': {'lr':0.03,'nl':80,'md':8,'ra':1.0,'rl':2.0,'mcs':20}, 'xgb': {'lr':0.03,'md':7,'ra':0.8,'rl':2.0,'mcw':5}, 'cb': {'lr':0.04,'d':7,'l2':3.0}},
]
...
for ps in param_sets:
    ...
    for ti,vi in KFold(5, shuffle=True, random_state=42).split(all_oof_np, y):
        m = Ridge(alpha=1.0); m.fit(all_oof_np[ti], y[ti])
        meta_oof[vi] = m.predict(all_oof_np[vi])
        meta_test += m.predict(all_test_np) / 5
    ss = r2_score(true, np.expm1(meta_oof))
    p(f"  Stacking: {ss*100:.4f}%")

    blend = np.clip(0.5*np.expm1(meta_oof)+0.5*np.clip(all_oof_np @ bw, 0, None), 0, None)
    bs = r2_score(true, blend)
    p(f"  Blend: {bs*100:.4f}%")

    best_score = max(ws, ss, bs)
    if bs >= ws and bs >= ss:
        fp = np.clip(0.5*meta_test+0.5*(all_test_np @ bw), 0, None)
    elif ss > ws:
        fp = np.clip(meta_test, 0, None)
    else:
        fp = np.clip(all_test_np @ bw, 0, None)

    all_preds[ps['name']] = fp
    all_oofs[ps['name']] = all_oof_np
    p(f"  Best: {best_score*100:.4f}%")

# ==========================================
# Cross-ensemble: use Set A's OOF to train meta on Set B's test and vice versa
# ==========================================
p(f"\n{'='*60}")
p("ENSEMBLE STRATEGIES:")

# 1. Simple average
avg_pred = (all_preds['sweet'] + all_preds['mid']) / 2
p(f"  Average of 2 sets")

# 2. Best single
best_single = 'sweet'
p(f"  Best single: {best_single}")

# Save both
sub = pd.DataFrame({'Index':test['Index'], 'demand':avg_pred})
sub.to_csv('submission_889.csv', index=False)
p(f"\nSaved submission_889.csv (avg ensemble)")

sub2 = pd.DataFrame({'Index':test['Index'], 'demand':all_preds['sweet']})
sub2.to_csv('submission_889_best.csv', index=False)
p(f"Saved submission_889_best.csv (sweet single)")
