...
p("=" * 60)
p("MAIN887 - main884 Config + 5 Seeds + CatBoost")
p("=" * 60)
...
# Optuna (smoothing=30, same as main884)
p("\nOptuna...")
...
def tune_cb(trial):
    p_ = {'iterations':500,'learning_rate':trial.suggest_float('lr',0.02,0.08),
    ...
    s = [r2_score(yv, CatBoostRegressor(**p_).fit(Xt,yt,eval_set=(Xv,yv),
         use_best_model=True).predict(Xv)) for Xt,Xv,yt,yv in fd]
    return np.mean(s)
...
st = optuna.create_study(direction='maximize')
st.optimize(tune_cb, n_trials=8, show_progress_bar=False)
best_cb = st.best_params
p(f"  CB:  {st.best_value:.6f}")

# Training: 5-fold x 5-seed, 3 models
N_FOLDS = 5
N_SEEDS = 5
p(f"\nTraining: {N_FOLDS}f x {N_SEEDS}s, 3 models")
...
sub.to_csv('submission_887.csv', index=False)
p(f"\nSaved submission_887.csv")
