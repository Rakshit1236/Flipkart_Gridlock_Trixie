...
p("=" * 60)
p("MAIN884 v2 - Enhanced Features + Optuna + Stacking")
p("=" * 60)
...
p(f"Features: {len(nc)}, TE cols: {te_cols}")

# ==========================================
# Optuna with pre-computed fold data (fast)
# ==========================================
p("\nOptuna (10 trials, 2-fold)...")
...
# ==========================================
# Full Training: 5-fold x 3-seed
# ==========================================
N_FOLDS = 5
N_SEEDS = 3
p(f"\nFull Training: {N_FOLDS}f x {N_SEEDS}s")
...
# Weight opt
def obj_fn(w):
    return -r2_score(true, np.clip(w[0]*np.expm1(oof_lgb)+w[1]*np.expm1(oof_xgb),0,None))
...
meta_oof_lin = np.zeros(len(train_main))
meta_test_lin = np.zeros(len(test))
for ti,vi in KFold(5, shuffle=True, random_state=42).split(stk, y):
    m = Ridge(alpha=1.0)
    m.fit(stk[ti], y[ti])
    meta_oof_lin[vi] = m.predict(stk[vi])
    meta_test_lin += m.predict(stk_t) / 5
ss = r2_score(true, np.expm1(meta_oof_lin))
p(f"Linear Stack: {ss*100:.4f}%")

# NL stacking
meta_oof_nl = np.zeros(len(train_main))
meta_test_nl = np.zeros(len(test))
for ti,vi in KFold(5, shuffle=True, random_state=42).split(stk, y):
    m = lgb.LGBMRegressor(n_estimators=200, learning_rate=0.03, num_leaves=15, max_depth=4,
        subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=0.1,
        min_child_samples=20, verbose=-1, n_jobs=-1, random_state=42)
    m.fit(stk[ti], y[ti], eval_set=[(stk[vi],y[vi])], callbacks=[lgb.early_stopping(20, verbose=False)])
    meta_oof_nl[vi] = m.predict(stk[vi])
    meta_test_nl += m.predict(stk_t) / 5
ns = r2_score(true, np.expm1(meta_oof_nl))
p(f"NL Stack: {ns*100:.4f}%")

# Blends
blend = np.clip(0.5*np.expm1(meta_oof_lin)+0.5*weighted_oof,0,None)
bs = r2_score(true, blend)
p(f"Blend: {bs*100:.4f}%")

scores = {'Weighted':ws, 'LinStack':ss, 'NLStack':ns, 'Blend':bs}
...
if best_name == 'Blend':
    fp = np.clip(0.5*np.expm1(meta_test_lin)+0.5*(bw[0]*np.expm1(t_lgb)+bw[1]*np.expm1(t_xgb)),0,None)
elif best_name == 'LinStack':
    fp = np.clip(np.expm1(meta_test_lin),0,None)
elif best_name == 'NLStack':
    fp = np.clip(np.expm1(meta_test_nl),0,None)
else:
    fp = np.clip(bw[0]*np.expm1(t_lgb)+bw[1]*np.expm1(t_xgb),0,None)

sub = pd.DataFrame({'Index':test['Index'], 'demand':fp})
sub.to_csv('submission_884.csv', index=False)
p(f"\nSaved submission_884.csv")
